...
=for comment
Classify each cluster based on RM custom library output
R --slave -e 'library(reshape2); write.table(recast(read.delim("seqClust/clustering/comparativeAnalysis_table_clusters_counts.csv"), cluster ~ sample, id.var = c("cluster", "sample"), measure.var="Freq"), sep="\t", row.names=F)' > comparativeAnalysis_table_clusters_counts.csv

 perl ../annotate.pl seqClust/clustering/RM-custom_output_tablesummary.csv |
     paste - comparativeAnalysis_table_clusters_counts.csv \
     > comparative_analysis_counts.annotated.txt

=cut 

# Use file seqClust/clustering/RM-custom_output_tablesummary.csv
