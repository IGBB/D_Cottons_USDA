...
#+BEGIN_SRC sh :var libs=read_groups[,3] :var DIR=(file-name-directory buffer-file-name) 
  ...
  parallel -j0 samtools index dedup/{.}.dedup.bam  <<< "$files"
...
#+HEADER: :prologue #PBS -N RealignerTargetCreator -l nodes=1:ppn=20 -l walltime=48:00:00 -t 1-31
#+BEGIN_SRC sh :var libs=read_groups[,3] :var DIR=(file-name-directory buffer-file-name) 
  swsetup () { eval `/usr/erc-share/etc/swsetup/swsetup.pl $*`; }
  swsetup samtools
...
#+BEGIN_SRC sh :var libs=read_groups[,3] :var DIR=(file-name-directory buffer-file-name) 
  ...
  parallel -j20 java -Djava.io.tmpdir=$TMP/{/.} -Xmx3g -jar $GATK -T IndelRealigner -R $DB \
           -I $ALIGN/dedup/${lib[0]}/dedup/${lib[0]}.dedup.bam \
           -targetIntervals $ALIGN/realign/${lib[0]}/${lib[0]}.{/.}.intervals \
           -o $ALIGN/realign/${lib[0]}/{/.}.realigned.bam '&>' $ALIGN/realign/${lib[0]}/${lib[0]}.indel_realigner.log <<<"$files"
...
#+BEGIN_SRC sh :var libs=read_groups[,3] :var DIR=(file-name-directory buffer-file-name) 
    ...
    cd $ALIGN/call/${lib[0]}
    variants=""
    for i in *.g.raw_variants.vcf; do variants+="--variant $i "; done
    pwd
    java -Djava.io.tmpdir=$TMP/${lib[0]} -Xmx3g -jar $GATK -T GenotypeGVCFs -R $DB \
        $variants \
        --out all.${lib[0]}.vcf \
        -stand_emit_conf 10 \
        -stand_call_conf 30  &> $ALIGN/call/${lib[0]}/joint.genotype.log
#+END_SRC
...
#+BEGIN_SRC sh :var libs=read_groups[,3] :var DIR=(file-name-directory buffer-file-name) 
    ...
    java -Djava.io.tmpdir=$TMP/${lib[0]} -Xmx3g -jar $GATK -T SelectVariants -R $DB \
        -V all.${lib[0]}.vcf \
        -selectType SNP \
        -o all.raw_snp.${lib[0]}.vcf &> $ALIGN/call/${lib[0]}/all.selectVariants_snp.eff

    java -Djava.io.tmpdir=$TMP/${lib[0]} -Xmx3g -jar $GATK -T SelectVariants -R $DB \
        -V all.${lib[0]}.vcf \
        -selectType INDEL \
        -o all.raw_indel.${lib[0]}.vcf &> $ALIGN/call/${lib[0]}/all.selectVariants_indel.eff

    java -Djava.io.tmpdir=$TMP/${lib[0]} -Xmx3g -jar $GATK -T VariantFiltration -R $DB \
        -V all.raw_snp.${lib[0]}.vcf \
        --filterExpression "QD<2.0||FS>60.0||MQ<40.0||SOR>4.0||MQRankSum<-12.5||ReadPosRankSum<-8.0" \
        --filterName all_snp_filter \
        -o all.filtered_snp.vcf &> all.VariantFiltration_snp.err

    java -Djava.io.tmpdir=$TMP/${lib[0]} -Xmx3g -jar $GATK -T VariantFiltration -R $DB \
        -V all.raw_snp.${lib[0]}.vcf \
        --filterExpression "QD<2.0||FS>200.0||SOR>10.0||ReadPosRankSum<-20.0" \
        --filterName indel_filter \
        -o all.filtered_indel.vcf &> all.VariantFiltration_indel.err

    java -Djava.io.tmpdir=$TMP/${lib[0]} -Xmx3g -jar $GATK -T SelectVariants -R $DB \
        -V all.filtered_snp.vcf \
        --excludeFiltered \
        -o all.PASS_snp.vcf &> all.PASS_snp.err

    java -Djava.io.tmpdir=$TMP/${lib[0]} -Xmx3g -jar $GATK -T SelectVariants -R $DB \
        -V all.filtered_indel.vcf \
        --excludeFiltered \
        -o all.PASS_indel.vcf &> all.PASS_indel.err
#+END_SRC
